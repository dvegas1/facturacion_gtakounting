...
function add_referencia(ref) {
    ...
    if(ref.includes("+")){
        ...
        var totalcanntidad = $("#"+valor+"_"+codalmacen_r).val();   

        $("#"+valor+"_"+codalmacen_r).val(totalcanntidad - 1);

        console.log($("#"+valor+"_"+codalmacen_r).attr('id'));
    ...
    var totalcanntidad = $("#"+ref+"_"+codalmacen_r).val();   

    $("#"+ref+"_"+codalmacen_r).val(totalcanntidad - 1);
...
var array = ["-1.jpg", "-1.png"];
var archivo_s = "";
var evaluar = false;

var nombres = "S/I";

var file = "images/articulos/" + archivo;
...
if (typeof (archivo) === "undefined") {
   ...
   for (let index = 0; index < array.length; index++) {
       ...
       $.get(String(file + array[index]), function (data, status) {



           if (data) {

               if (typeof (file + array[index]) !== "undefined") {
                   console.log(file + array[index]);
                   return String(file + array[index]);
               }
           }
   ...
   for (let index = 0; index < array.length; index++) {
       ...
       nombres = String(file + array[index]);
       console.log("nombre : " + nombres + "  indice " + index + " indarray " + array[index]);
       $.get(nombres)
           .done(function () {
               //   console.log("existe: " + file + array[index]);


               console.log("Existe: " + nombres);


           }).fail(function () {


               console.log("Not existe: " + file + array[index]);
   ...
   /*
       ...
       $.ajax({
           ...
           success: function () {
               ...
               console.log("imagen encontrado " + archivos);
               evaluar = true;
       ...
       $.ajax({
           ...
           success: function () {
               ...
               evaluar = true;
               console.log("imagen encontrado " + archivos_1);
...
function mostrar_factura(id) {
    ...
    function b_search_img(datos, callback) { 
    ...
    $.ajax({
        ...
        success: function (datos) {
           ...
           callback(null,data);
            ...
            //localStorage.setItem("img_ref", JSON.stringify(datos));
            //valor_img_ref = localStorage.setItem("img_ref", JSON.stringify(datos));
...
function buscar_articulos() {
    ...
    if (document.f_buscar_articulos.query.value == '') {
        ...
        $.getJSON(tpv_url, $("form[name=f_buscar_articulos]").serialize(), function (json) {
            ...
            b_search_img(json, function(errorLanzado, datosDevueltos){
            ...
            $.each(json, function (key, val) {             
                ...
                if (val.sevende && (val.stockalm > 0 || val.controlstock)) {
                    ...
                    if(val.disponible < 1){
                        ...
                        <a href=\"#\" aria-hidden='true' data-toggle='popover' data-toggle='popover' data-html='true'  data-img=" + "'" +  val.imagen + "'" +   " onclick=\"" + funcion + "\">" + val.referencia + '</a> ' + " <span    class='glyphicon glyphicon-picture' aria-hidden='true' data-toggle='popover' data-html='true'  data-img=" + "'" +  val.imagen + "'" + "></span> &nbsp" + val.descripcion + "</td>\n\
                          <td class=\"text-right\"><a href=\"#\" onclick=\"" + funcion + "\">" + show_pvp_iva(val.pvp * (100 - val.dtopor) / 100, val.codimpuesto) + "</a></td>\n\<td class=\"text-left a_red\">"+"</a class='a_red'>"+val.nombre+"</a>"+"</td>\n\<td class=\"text-left\">"+" <a aria-hidden='true' data-html='true' '</a class='a_red'>"+val.disponible+"</td></tr>");

                        }else{


                        items.push(tr_aux + "<td>\n\
                        <a href=\"#\" aria-hidden='true' data-toggle='popover' data-toggle='popover' data-html='true'  data-img=" + "'" +  val.imagen + "'" + "onclick=\"" + funcion + "\">" + val.referencia + '</a> ' + " <span    class='glyphicon glyphicon-picture' aria-hidden='true' data-toggle='popover' data-html='true'  data-img=" + "'" +   val.imagen + "'" +  "></span> &nbsp"
                                + val.descripcion + "</td>\n\
                          <td class=\"text-right\"><a href=\"#\" onclick=\"" + funcion + "\">" + show_pvp_iva(val.pvp * (100 - val.dtopor) / 100, val.codimpuesto) + "</a></td>\n\<td class=\"text-left\">"+"</a>"+val.nombre+"</td>\n\<td class=\"text-left\">"+" <a aria-hidden='true' data-html='true' '</a >"+val.disponible+"</td></tr>");
                    ...
                    if(val.disponible < 1){
                        ...
                        items.push(tr_aux + "<td>\n\
                        <a href=\"#\" aria-hidden='true' data-toggle='popover' data-toggle='popover' data-html='true'  data-img=" + "'" +  val.imagen + "'" + "onclick=\"alert('Sin stock.')\">" + val.referencia + '</a> ' + " <span    class='glyphicon glyphicon-picture' aria-hidden='true' data-toggle='popover' data-html='true'  data-img=" + "'" +   val.imagen + "'" +  "></span> &nbsp"
                                + val.descripcion + "</td>\n\
                          <td class=\"text-right\"><a href=\"#\" onclick=\"" + funcion + "\">" + show_pvp_iva(val.pvp * (100 - val.dtopor) / 100, val.codimpuesto) + "</a></td>\n\<td class=\"text-left\">"+"</a>"+val.nombre+"</td>\n\<td class=\"text-left\">"+" <a aria-hidden='true' data-html='true' '</a >"+val.disponible+"</td></tr>");
                ...
                items.push(tr_aux + "<td>\n\
                <a href=\"#\" aria-hidden='true' data-toggle='popover' data-html='true'  data-img=" +  "'" +  val.imagen + "'" + "onclick=\"alert('Sin stock.')\">" + val.referencia + '</a>' + "&nbsp <span   id='span_img_data' class='glyphicon glyphicon-picture' aria-hidden='true' data-toggle='popover' data-html='true'  data-img=" + "'" +   val.imagen + "'" + "></span> &nbsp" + val.descripcion + "</td>\n\
                <td class=\"text-right\"><a href=\"#\" onclick=\"alert('Sin stock.')\">" + show_pvp_iva(val.pvp * (100 - val.dtopor) / 100, val.codimpuesto) + "</td>\n\<td class=\"text-left\">"+"</a>"+val.nombre+"</td>\n\<td class=\"text-left\">"+" <a aria-hidden='true' data-html='true' '</a>"+val.disponible+"</td></tr>");
            ...
            if (insertar) {
                $("#search_results").html("<div class=\"table-responsive tb_arti_search\"><table class=\"table table-hover\"><thead><tr>\n\
               <th class=\"text-left\">Referencia + descripción</th><th class=\"text-right\">Precio</th>\n\
               <th class=\"text-left\">Almacen</th><th class=\"text-left\">Disponible</th></tr></thead>" + items.join('') + "</table></div>");
            }
        });
    }
}
