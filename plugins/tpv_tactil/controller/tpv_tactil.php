...
class tpv_tactil extends fbase_controller
{
    ...
    private function caja_iniciada()
    {
        ...
        if (isset($_POST['idtpv_comanda'])) {
            ...
            if ($this->comanda) {
             ...
             if (isset($_POST['get_archivo'])) {          
            
            $artic_0 = new articulo();
            $this->template = FALSE;
            header('Content-Type: application/json');


            echo json_encode($artic_0->validar_image_url($_POST['get_archivo']));
               ...
               if (isset($_REQUEST['add_ref_validar'])) {
               $this->template = FALSE;
               header('Content-Type: application/json');
               $val = new articulo();
               $val->almacencode = $this->terminal->codalmacen;


              echo $val->validarProduct($_REQUEST['add_ref_validar']);
    ...
    private function add_ref_validar()
    {
        $this->template = 'ajax/tpv_tactil_lineas';
        $this->resultado = array();
