...

{
...

    {
        ...
        if (!isset(self::$impuestos)) {
            ...
            /*self::$column_list_e = 'T1.referencia,T1.codfamilia,T1.codfabricante,T1.descripcion,T1.pvp,T1.factualizado,T1.costemedio,T1.preciocoste,T1.codimpuesto,T1.stockfis,T1.stockmin,T1.stockmax,

            T1.controlstock,T1.nostock,T1.bloqueado,T1.secompra,T1.sevende,T1.equivalencia,T1.codbarras,T1.observaciones,T1.imagen,T1.publico,T1.tipo,T1.partnumber,T1.codsubcuentacom,

            T1.codsubcuentairpfcom,T1.trazabilidad,

            T1.referencia,T2.codalmacen as almacen,IF(T2.nombre="", "ALMACEN S/N", T2.nombre) as nombre,T2.disponible as disponible';*/
    ...
    public function sinoconstrock($REF){
    ...
    public function validarProduct($REF) {
      ...
      $campos="SUM(STO.disponible) as total";



      $query="SELECT " . $campos . " FROM articulos ar,stocks sto,familias fa, almacenes al

      WHERE ar.referencia = sto.referencia AND ar.codfamilia = fa.codfamilia AND al.codalmacen = sto.codalmacen AND al.codalmacen = " ."'" . $this->almacencode . "'" . "

      AND ar.referencia = " ."'" . $REF . "'" . " ORDER BY STO.cantidad desc;";
      ...
      $valdisponible = "SELECT SUM(disponible)  as disponible FROM stocks WHERE referencia ="  . "'" . $REF . "'" . " AND  codalmacen = " ."'" . $this->almacencode . "'" . ";";



      $valdisponiblef=$this->db->select($valdisponible);





      $validar = $valdisponiblef[0]['disponible'];
    ...
    public function disponible_stock($art){



        //  $datapreciocoste= $this->db->select("SELECT sum(cantidad) as total from stocks WHERE referencia = " . "'"  . $REF . "'" . ")");

    

        /*  $campos="SUM(STO.cantidad) as total1";

          $datapreciocoste= $this->db->select("SELECT " . $campos . " FROM articulos ar,stocks sto,familias fa

          WHERE ar.referencia = sto.referencia AND ar.codfamilia = fa.codfamilia

          AND ar.referencia = " ."'" . $REF . "';");*/

    

          $campos="al.nombre as nombre,STO.cantidad as cantidad,ar.referencia referencia,sto.codalmacen almacen";

    

          $query="SELECT " . $campos . " FROM articulos ar,stocks sto,familias fa, almacenes al

          WHERE ar.referencia = sto.referencia and ar.codfamilia = fa.codfamilia and al.codalmacen = sto.codalmacen
          ...
          $datapreciocoste= $this->db->select($query);

         

          $this->totalcantidad= $datapreciocoste; 



          return $datapreciocoste;
    }

    public function api_validarStock($REF){
    ...
    public function validarStock($REF){
    ...
    /*  $campos="SUM(STO.cantidad) as total1";
      ...
      $campos="al.nombre as nombre,sto.cantidad as cantidad,ar.referencia as referencia,sto.codalmacen as almacen,ar.descripcion descripcion";
    ...
    //  $this->totalcantidad =  json_encode($datapreciocoste);



  //    return  $datapreciocoste[0]['total'];
     ...
     */
         ...
         public function validar_image_url($image)

    {
        ...
        if (file_exists(FS_MYDOCS . 'images/articulos/' . $image[$i] . '-1.png')) {

             $valor[$i]='images/articulos/' . $image[$i] . '-1.png';

        } else if (file_exists(FS_MYDOCS . 'images/articulos/' . $image[$i] . '-1.jpg')) {

             $valor[$i]='images/articulos/' . $image[$i] . '-1.jpg';
            ...
            for ($i=0; $i < count($valor); $i++) {             
                ...
                $sql="UPDATE articulos SET imagen =" . "'" .  $valor[$i] . "'" . "WHERE  referencia =" . "'". $image[$i] ."'".  ";";
...

    {
        ...
        if (count($artilist) <= 1) {
            ...
            T1.referencia,T1.codfamilia,T1.codfabricante,T1.descripcion,T1.pvp,T1.factualizado,T1.costemedio,T1.preciocoste,T1.codimpuesto,T1.stockfis,T1.stockmin,T1.stockmax,

            T1.controlstock,T1.nostock,T1.bloqueado,T1.secompra,T1.sevende,T1.equivalencia,T1.codbarras,T1.observaciones,T1.publico,T1.tipo,T1.partnumber,T1.codsubcuentacom,T1.imagen as imagen,

            T1.codsubcuentairpfcom,T1.trazabilidad,

            T1.referencia,T2.codalmacen as almacen,IF(T2.nombre='', 'ALMACEN S/N', T2.nombre) as nombre,T2.disponible as disponible FROM  articulos T1 INNER JOIN stocks T2 ON T1.referencia = T2.referencia";
            ...
            if ($codfamilia != '') {
                ...
                $sql .= $separador . " T1.codfamilia = " . $this->var2str($codfamilia);
            ...
            if ($codfabricante != '') {
                ...
                $sql .= $separador . " T1.codfabricante = " . $this->var2str($codfabricante);
            ...
            if ($bloqueados) {
                ...
                $sql .= $separador . " T1.bloqueado = TRUE";

                $separador = ' AND';

            } else {

                $sql .= $separador . " T1.bloqueado = FALSE";
            ...
            if ($query == '') {
                ...
                $sql .= $separador . " (T1.referencia = " . $this->var2str($query)

                    . " OR T1.referencia LIKE '%" . $query . "%'"

                    . " OR T1.partnumber LIKE '%" . $query . "%'"

                    . " OR T1.equivalencia LIKE '%" . $query . "%'"

                    . " OR T1.descripcion LIKE '%" . $query . "%'"

                    . " OR T1.codbarras = " . $this->var2str($query) . ")";
                ...
                if (count($palabras) > 1) {
                    ...
                    $sql .= $separador . " (T1.lower(referencia) = " . $this->var2str($query)

                        . " OR T1.lower(referencia) LIKE '%" . $query . "%'"

                        . " OR T1.lower(partnumber) LIKE '%" . $query . "%'"

                        . " OR T1.lower(equivalencia) LIKE '%" . $query . "%'"
                    ...
                    foreach ($palabras as $i => $pal) {
                        ...
                        if ($i == 0) {
                            ...
                            $sql .= "T1.lower(descripcion) LIKE '%" . $pal . "%'";

                        } else {

                            $sql .= " AND lower(T1.descripcion) LIKE '%" . $pal . "%'";
                    ...
                    $sql .= $separador . " (lower(T1.referencia) = " . $this->var2str($query)

                        . " OR lower(T1.referencia) LIKE '%" . $query . "%'"

                        . " OR lower(T1.partnumber) LIKE '%" . $query . "%'"

                        . " OR lower(T1.equivalencia) LIKE '%" . $query . "%'"

                        . " OR lower(T1.codbarras) = " . $this->var2str($query)

                        . " OR lower(T1.descripcion) LIKE '%" . $query . "%')";
            ...
            if (strtolower(FS_DB_TYPE) == 'mysql') {
                ...
                $sql .= " ORDER BY lower(T1.referencia) ASC";

            } else {

                $sql .= " ORDER BY T1.referencia ASC ";
