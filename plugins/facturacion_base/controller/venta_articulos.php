...
class venta_articulos extends fbase_controller
{
    ...
    {
        parent::__construct(__CLASS__, 'Venta articulos', 'informes');
    }
    ...
    private function ini_filters()
    {
        ...
        if (isset($_POST['solodia'])) {

            $this->solodia = $_POST['solodia'];
        ...
        if (isset($_POST['desde'])) {
            ...
            $this->dia = substr($_POST['desde'], 0,2); 

            $this->mes = substr($_POST['desde'], 3,2); 

            $this->año = substr($_POST['desde'], 6,10); 
    ...
    private function top_articulo_faccli()
    {
        ...
        $alm_cod = "";
        $alm_cod_res = "";
        $alamceQ = $this->codalmacen;       

   // echo $alamceQ;

   $this->codalmacen_temp = "<> ''";
        foreach ($this->almacenes as $alm) {         
            ...
            $valoresalmacen = trim($alm_cod, ',');
             ...
             if($this->codalmacen== "Todos") {

             $this->almacenselect = $valoresalmacen;
            
            $alamceQ = " IN (" . $valoresalmacen . ")";
       
            $this->codalmacen_temp= $alamceQ;

            
            }else{
                $this->codalmacen_temp = " = " .  "'" . $this->codalmacen  . "'";
            }
        ...
        if (!$toplist || isset($_POST['desde'])) {
            $toplist = array();
            $articulo = new articulo();
            $sql = "SELECT age.nombre as agente,l.referencia, SUM(l.cantidad) as unidades, SUM(l.pvptotal/f.tasaconv) as total,f.codalmacen as almacen,
            substr(f.fecha,9,8) as dia, f.fecha as fecha"
                . " FROM lineasfacturascli l, facturascli f, agentes age "
                . " WHERE l.idfactura = f.idfactura AND l.referencia IS NOT NULL AND age.codagente = f.codagente"
                . " AND substr(f.fecha,1,4) = " . $this->año 
                . " AND substr(f.fecha,6,2) = " . $this->mes
                . " AND f.codalmacen  " .  $this->codalmacen_temp;

                if($this->solodia ==""){
                    
                    $sql.= " AND substr(f.fecha,9,8) = " . $this->dia 
                    . " AND f.codagente " . $agente
                    . " GROUP BY referencia ORDER BY unidades DESC";

                }else{

                    $sql.=  " AND f.codagente " . $agente
                    . " GROUP BY referencia ORDER BY unidades DESC";
