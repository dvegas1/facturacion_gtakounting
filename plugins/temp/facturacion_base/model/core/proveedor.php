...
class proveedor extends \fs_model
{
       ...
       */
          ...
          public function agregar_sugerencias($sugerencia,$sugerido,$tipo,$id)
      {

        if($id==2){
          $sql_d= "delete from sugerencias_proveedor";
          if(!$this->db->exec($sql_d)){
             return false;
          }
        }
        $sugerencias_a = explode(",", $sugerencia);
        $sugerido_a = explode(",", $sugerido);
        $sugerencia_agregada ="";

        if(count($sugerencias_a) == count($sugerido_a)){

        $sql = "INSERT INTO sugerencias_proveedor " . " (usuario,tipo,palabra,palabra_sugerida) VALUES ( ";

        $sql .=  "'DVEGAS'," . "'".$tipo."',";

        for ($i=0; $i < count($sugerencias_a); $i++) {

        if($sugerencias_a[$i] != "" && $sugerido_a[$i] != ""  && strlen($sugerencias_a[$i]) > 0 && strlen($sugerido_a[$i]) > 0) {

        if($sugerencias_a[$i] != "" && $sugerido_a[$i] != ",") {

         $sugerencia_agregada = $sugerido_a[$i];

        $sql .=  "'".$sugerencias_a[$i]."'," . "'".$sugerido_a[$i]."'";

        if($i < (count($sugerencias_a)-1) ) {
     ...
     echo "Datos incorrectos.";
      ...
      public function obtener_sugerencias($array_1)
      ...
      $new.= "  lower(palabra) in (";
       ...
       $array = str_replace($this->remplaze, "", $array_1);
       ...
       for ($i=0; $i < count($array); $i++) {
        ...
        //echo "SELECT palabra_sugerida FROM sugerencias_proveedor  WHERE " . $final_q. ";\n";


        //$this->db->select("SELECT palabra_sugerida FROM sugerencias_proveedor  WHERE " . $final_q. ";");
         $this->sugerencias_nuevas = $this->db->select("SELECT palabra_sugerida FROM sugerencias_proveedor  WHERE " . $final_q. ";");
...

    {
      ...
      $sincoma.= $rest . ")";
         ...
         //$this->result_bloque="SELECT * FROM parabas_bloqueadas WHERE " .$sincoma. ";";

        //$this->result_bloque =$sincoma;


        $data=$this->db->select("SELECT palabra FROM parabas_bloqueadas WHERE " .$sincoma. ";");


        if ($data) {
          $this->result_bloque= $data[0];
        }
        //var_dump($this->result_bloque);


    }

          public function get_proveedor($ref)
      {
        ...
        $this->bloqueo_palabras_ocr($array);
          ...
          if(is_array($this->result_bloque) && count($this->result_bloque) > 0) {
        ...
        $nuevas_sugerencias = array();
        $nuevas_sugerencias = $this->sugerencias_nuevas;

        //return $nuevas_sugerencias;

                  if(is_array($nuevas_sugerencias) && count($nuevas_sugerencias) > 0) {
      ...
      if($string_query != "" && strlen($string_query) > 3){
        ...
        $new .= " lower(nombre) like lower('%".strtolower($string_query)."%') ";
  ...
  $data = $this->db->select("SELECT " . $campos . " FROM proveedores WHERE  " . $final_q. ";");
    ...
    return $result_oct;
      ...
      public function all_full()
      {
          ...
          if (!$provelist) {
              ...
              /// guardamos la lista en la cache
