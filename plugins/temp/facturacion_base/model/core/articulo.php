...
class articulo extends \fs_model
{
       ...
       */
      public $tipo;
      public $sugerencias_nuevas;
      public $remplaze;
      public $result_bloque;
      ...
      public function __construct($data = FALSE)
      {
          ...
          $this->pvp_ant = 0.0;
          $this->iva = NULL;
          $this->sugerencias_nuevas = array();
          $this->remplaze = ["$","?"];
          $this->result_bloque = array();
      }
      ...
      public function cargar_palabras($tipo)
      {

              if($tipo=="0") {
                      $data = $this->db->select("SELECT tipo,palabra,palabra_sugerida FROM sugerencias_proveedor");
              }

              if($tipo=="1") {
                $data = $this->db->select("SELECT palabra FROM parabas_bloqueadas");
              }
...
function Agregar_bloquadas_ocr($palabra,$accion)
      {
        $array_sql =array();
        $palabras = explode(",", $palabra);

        if(count($palabras) ==0){
          return "Sin palabras agregadas.";
        }
        ...
        $repetidas_S =array();
        $aceptadas_S = array();
        $mensaje="Palabras agregada con exito.";

        if($accion==2) {
        $sql="DELETE FROM parabas_bloqueadas";

        if(!$this->db->exec($sql)) {
          return "";
        }else{
          $mensaje =" Palabras  Modificadas con exito. ";
        }
          }
              ...
              for ($i=0; $i < count($palabras); $i++) {
                ...
                $sql_val="SELECT * FROM parabas_bloqueadas WHERE palabra =";
                $sql_val.="'".$palabras[$i]."'" . ";";

                if(!$this->db->select($sql_val)) {
                //  return $sql_val;

                $sql = "INSERT INTO parabas_bloqueadas " . " (usuario,palabra) VALUES ( ";

                $sql .=  "'DVEGAS',";

              if($palabras[$i] != "")  {

               $palabras_f[$i] = $palabras[$i];
               $aceptadas.= $palabras[$i] . ", ";
    ...
    //  for ($i=0; $i < count($array_sql); $i++) {
      ...
      /*  if($this->db->exec($array_sql[$i])){
          $contador++;
        }*/

        foreach ($array_sql as &$valor) {
          if($this->db->exec($valor)){
            $contador++;
          }
        }
      ...
      if($contador > 0 && $repetidas != ""){


        $mensaje  ="Palabras agregas : " . $aceptadas;
        $mensaje.= "Palabras Rechazadas :  " . $repetidas.".";
      ...
      return $mensaje;
        ...
        function obtener_sugerencias($array_1){

        $new="";
        $new.= "  lower(palabra) in (";

        $remplaze=["'",'‘'];

        $array = str_replace($remplaze, "", $array_1);

        for ($i=0; $i < count($array); $i++) {

          $array_val_min = strtolower($array[$i]);
          ...
          $sincoma =  rtrim($final_q, ',');

          $final_q.=" )";


          $data = $this->db->select("SELECT palabra_sugerida FROM sugerencias_proveedor  WHERE " . $final_q. ";");


        //  return "SELECT palabra_sugerida FROM sugerencias_proveedor  WHERE " . $final_q. ";";

          $this->sugerencias_nuevas = $data;



        }

        public function bloqueo_palabras_ocr($array)
      ...
      //  $array = explode(",", $ref);
        ...
        //  return "SELECT palabra FROM parabas_bloqueadas  WHERE " . $final_q. ";";


        //  $data = $this->db->select("SELECT palabra FROM parabas_bloqueadas  WHERE " . $final_q. ";");


      }


          public function get_articulo_ocr($ref)
      {

              $array = explode(",", $ref);
              $array = str_replace($this->remplaze, "", $array);
              $this->bloqueo_palabras_ocr($array);
              $array_res = $this->result_bloque;
              ...
              $this->obtener_sugerencias($array);

              $nuevas_sugerencias = $this->sugerencias_nuevas;
              ...
              //$palabra = array_column($sugerencias, 'palabra');
              $nuevas_sugerencias = array_column($nuevas_sugerencias, 'palabra_sugerida');

              if(is_array($nuevas_sugerencias)){
              for ($i=0; $i < count($nuevas_sugerencias); $i++) {

                if($nuevas_sugerencias[$i] != "") {
                array_push($array, $nuevas_sugerencias[$i]);
                }
                  }
                    }
        ...
        $campos="descripcion,referencia,pvp,codimpuesto";
        ...
        for ($i=0; $i < count($array); $i++) {
          ...
          if($array[$i] != ""){
            ...
            $string_query=preg_replace('/[^A-Za-z0-9\-]/', '', $array[$i]); // Removes special chars.


        if(strlen($array[$i]) >= 3){
            $new .= " lower(descripcion) like lower('%".$string_query."%') ";
  ...
  $ocr_or_sup = substr($new, -2, 10);

  $rest = substr($new, -4, 4);

  if ($rest==" OR "){

   $final_q = substr($new,0,-4);

  }else{
   $final_q = $new;
  }

  //echo "SELECT " . $campos . " FROM articulos WHERE" . $final_q. ";";

//  return "SELECT " . $campos . " FROM articulos WHERE" . $final_q. ";";

  $data = $this->db->select("SELECT " . $campos . " FROM articulos WHERE" . $final_q. ";");
...
//return "SELECT " . $campos . " FROM articulos WHERE" . $final_q. ";";
  if($data){
    ...
    return $data;
       ...
       $string_query=preg_replace('/[^A-Za-z0-9\-]/', '', $string); // Removes special chars.

       if($string_query != "" && strlen($string_query) > 3){

           $result_oct .= $string_query . ",";
           }
        }
