...
class tpv_tactil extends fbase_controller
{
    ...
    protected function private_core()
    {
        ...
        $this->ultentregado = 0;
        $this->ultventa = 0;
        $this->cajas = $this->fbase_sql_distinct('clientes','nombre');
        $this->Rescajas = array();
        $this->impresion= array();
        $this->impresiontemp= array();
        $this->imprevalor =FALSE;
        $this->nombre ="";
        ...
        if (isset($_REQUEST['buscar_cliente'])) {
            echo $this->fbase_buscar_cliente($_REQUEST['buscar_cliente']);

        } else if (isset($_REQUEST['codbar2'])) {
            $this->buscar_articulo();

        } else if (isset($_REQUEST['cajas'])) {
            $this->getcajas();


        } else if (isset($_REQUEST['idtpv_comanda_impresion'])) {

            $this->Actualizar_impresion();



        } else if (isset($_REQUEST['actualizarImpresion'])) {
             $this->actualizarImpresion();


         } else if (isset($_REQUEST['cajasSeleccion'])) {
            echo $this->fbase_buscar_cliente($_REQUEST['cajasSeleccion']);
      ...
      private  function Actualizar_impresion()
        {
          $linea_comanda = new linea_comanda();
          $linea_comanda->actualizar_comanda_impresa($_REQUEST['idtpv_comanda_impresion']);
           echo ("comanda recibida" . $_REQUEST['idtpv_comanda_impresion']);
    ...
    private function caja_iniciada()
    {
        ...
        if (isset($_POST['tp_seguridad'])) {

        $this->template = FALSE;
        $comanda13 = new tpv_comanda();
       header('Content-Type: application/json');
        echo json_encode($comanda13->segurity_tpv($_POST['tp_seguridad'],$_POST['accion']),TRUE);

       }

       if (isset($_POST['tp_eliminar'])) {

       $this->template = FALSE;
       $comanda14 = new tpv_comanda();
       header('Content-Type: application/json');
       echo json_encode($comanda14->tp_validar_eliminadas($_POST['tp_eliminar']));
    ...
    function actualizarImpresion(){
                             ...
                             $linea->idtpv_comanda = $_REQUEST['actualizarImpresion'];

                             if($linea->save_impresion()){
    ...
    private function guardar_ticket()
    {
        ...
        if ($continuar) {
            ...
            $comanda->nombrecliente = $cliente->nombre;
            ...
            if(isset($_POST['codimpresion'])){
             $linea = new linea_comanda();
             if ($linea->save()) {
              echo "Gurdando comanda";
             }
            }
                   ...
                   //     echo $impresionsi;
                    $art0 = $articulo->get($_POST['referencia_' . $i]);
                      ...
                      if ($art0) {
                            ...
                            if (isset($_POST['codcombinacion_' . $i])) {$linea->codcombinacion = $_POST['codcombinacion_' . $i];
                            }
                        //    echo strval($_POST['impresion_' . $i]);
                          if(!empty($_POST['impresion_' . $i])){
                            $linea->impresion=strval($_POST['impresion_' . $i]);
                          }
    ...
    private function generar_factura(&$comanda, &$ejercicio, &$serie, &$divisa, &$forma_pago, &$cliente)
    {
        ...
        if ($factura->save()) {
            ...
            foreach ($comanda->get_lineas() as $lin) {
                ...
                $linea->impresion = $this->impresion;
                $linea->save();
            }
