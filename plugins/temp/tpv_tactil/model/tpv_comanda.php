...
class tpv_comanda extends fs_model
{
    ...
    public function getname()
    {
      ...
      $data ="SELECT nombre FROM clientes WHERE codcliente=";
      $data .= "'".$this->codcliente."'" . ";";



      $valor=$this->db->select($data);

      if(count($valor) > 0){
          return new tpv_comanda($valor[0]);
    ...
    public function get_lineas()
    {
      ...
      if($this->estadoImpresion== 0){
        $lineac = new linea_comanda();
        return $lineac->all_from_comanda($this->idtpv_comanda);
      }

      if($this->estadoImpresion== 1){
        $lineac = new linea_comanda();
        return $lineac->all_from_comanda_impresion($this->idtpv_comanda);
    ...
    function tp_validar_eliminadas($comanda)
    {

        $query="SELECT idtpv_comanda FROM tpv_lineascomanda WHERE impresion > 0 AND  idtpv_comanda=" . "'" . $comanda . "'" . ";";
    ...
    function segurity_tpv($array,$accion)
    {
      ...
      switch ($accion) {
        ...
        $sqld="DELETE FROM tp_passwords_tpv";

        if(!$this->db->exec($sqld)) {
        ...
        for ($i=0; $i < count($array); $i++) {
          ...
          $sql[$i] = "INSERT INTO tp_passwords_tpv (password) VALUES ( " . "'" . $array[$i] . "'" . ");";
      ...
      if($contador > 0 ) {
        ...
        $data = $this->db->select("SELECT password FROM tp_passwords_tpv");
        ...
        $data ="SELECT password FROM tp_passwords_tpv where password=";
        $data .= "'".$array."'" . ";";
    ...
    public function get_lineas_impresion()
    {
