...
class tpv_comanda extends fs_model
{
    ...
    public function delete()
    {
      ...
      $ACTUALIZARSTOCK= "SELECT li.idtpv_comanda as nrocomanda,li.referencia as referencia,li.cantidad as cantidad,co.codalmacen as codalmacen FROM tpv_comandas co LEFT JOIN tpv_lineascomanda li on co.idtpv_comanda = li.idtpv_comanda
      WHERE li.idtpv_comanda = " . $this->var2str($this->idtpv_comanda) . ";";

      $data = $this->db->select($ACTUALIZARSTOCK);
      ...
      if ($data) {
        ...
        foreach ($referencia as $key => $value) {
            ...
            $query[] = "UPDATE stocks set   cantidad = "  . " cantidad + " . $cantidad[$key]  . ", disponible = disponible + " . $cantidad[$key]  . "  WHERE referencia = " . "'" . $referencia[$key] . "'"
            . "  AND codalmacen = " . "'" . $codalmacen[$key] . "';";

        }

        foreach ($query as $value) {

          if(!$this->db->exec($value)){
              return false;
          }
